...
class ExperimentService:
    ...
    def convert_experiment_entities_for_user_interface(experiment_entities: List[ExperimentEntity], 
        ...
        for index, experiment in enumerate(sorted_experiment_entities):
            ...
            experiment_response = GetExperimentsResponse(id=experiment.id,
                                                         ...
                                                         total_cluster_units=sample_size,
    ...
    @staticmethod
    def get_cluster_units_from_sample_entity(sample_id: PyObjectId, only_return_ids: Optional[bool]=False) -> List[ClusterUnitEntity] | List[PyObjectId]:
        sample_entity = get_sample_repository().find_by_id(sample_id)

        if not sample_entity:
            raise Exception(f"the provided sample of the experiment_entity not exist| sample_id: {sample_id}")
        
        if not sample_entity.sample_cluster_unit_ids:
            raise Exception(f"No cluster units have been assigned in the sample for us to do an experiment with | sample_id: {sample_id}")
        
        if only_return_ids:
            return sample_entity.sample_cluster_unit_ids
    ...
    def get_input_cluster_unit_entities_from_expertiment(experiment_entity: ExperimentEntity, only_return_ids: Optional[bool]=False) -> List[ClusterUnitEntity] | List[PyObjectId]:
        ...
        if experiment_entity.input.input_type == "sample":
            ...
            if only_return_ids:
                return filtering_entity.output_cluster_unit_ids

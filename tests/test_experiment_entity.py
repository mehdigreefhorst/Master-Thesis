...
def test_prediction_result_defaults():
    ...
    assert result.prevelance_distribution == {}
...
def test_experiment_with_custom_aggregate():
    ...
    custom_aggregate = AggregateResult(
        problem_description=PredictionResult(
            prevelance_distribution={"3": 120, "2": 40, "1": 10, "0": 100},  # Keys must be strings
    ...
    assert experiment.aggregate_result.problem_description.prevelance_distribution["3"] == 120


def test_prevalence_dict_validates_numeric_keys():
    """Test that prevelance_distribution only accepts numeric string keys"""
    import pytest

    # Should work with numeric strings
    valid_result = PredictionResult(
        prevelance_distribution={"0": 5, "1": 10, "2": 15}
    )
    assert valid_result.prevelance_distribution["0"] == 5

    # Should fail with non-numeric strings
    with pytest.raises(ValueError, match="Dictionary keys must be numeric strings"):
        PredictionResult(
            prevelance_distribution={"invalid": 5, "0": 10}
        )
